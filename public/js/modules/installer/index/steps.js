    	...
    	if($("#frmInstallStep4").valid()){
    		$.ajax({
                ...
                success: function(data) {
                    if(data['success']==true){
                    	//mark selected step in the legend
                        $('#semiotic').load('/installer/index/loadsemiotic');
                    	notSelectedStep(4);
                        selectedStep(5);
                    	$('#step4_container').addClass('hide');
    ...
    $('#step5_next').bind('click',function(){
       if($('#frmInstallStep5').valid()){
           $.ajax({
                type: 'POST',
                async: false,
                url: '/installer/index/semiotics',
                dataType: 'json',
                data: 	$( "#frmInstallStep5" ).serialize(),
                success: function(data) {													
                    if(data['success'])
                    {
                        $('#step5_container').addClass('hide');
    	...
    	if($("#frmInstallStep6").valid()){
    		if($('#config_type').val()=='express'){
	            ...
	            $.ajax({
	                ...
	                dataType: 'html',
	                success: function(data) {
	                    $('#step7_container').html(data);
	                    $('#step6_container').addClass('hide');
	                    $('#step7_container').removeClass('hide');
	                    //mark selected step in the legend
	    			    notSelectedStep(6);
	    		        selectedStep(7);
