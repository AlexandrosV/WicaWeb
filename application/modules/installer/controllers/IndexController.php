...
//				'es'=>'Espa&ntilde;ol',
	...
	public function step6Action(){
        ...
        /**
         * Gets data WC_ONTOLOGY
         */
        public function loadsemioticAction(){
            //disable autorendering for this action
            $this->_helper->layout->disableLayout ();
//            $this->_helper->viewRenderer->setNoRender();
            
            $ont_obj = new Ontology_Model_Ontology();
            $ont_list_obj = $ont_obj->find('wc_ontology', array('section_id'=>''));
        ...
        /**
         * Get data from installer and stores it in database
         */
        public function semioticsAction(){
            //disable autorendering for this action
            $this->_helper->layout->disableLayout ();
            $this->_helper->viewRenderer->setNoRender();
	        ...
	        $semiotic_obj = $semiotic->getNewRow('wc_ontology');
                
                $j=0;
                $count = $formData['count'];
                for ($i = 1; $i <= $count; $i++){
                    $semiotic_obj->id = $formData['id_'.$i];
                    $semiotic_obj->original = $formData['original_'.$i];
                    $semiotic_obj->semiotic = $formData['semiotic_'.$i];
                    $semiotic_obj->section_id = NULL;
                    
                    $saved = $semiotic->save('wc_ontology',$semiotic_obj);
